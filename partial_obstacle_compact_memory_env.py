...
class PartialObstacleCompactMemoryEnv(gym.Env):
    """
    Observation
    -----------
    1. 밝혀진 9x9 지도             : 81
    ...
    def __init__(self):
        ...
        # Compact observation (18):
        # 0..8   : local 3x3 obstacle/boundary information
        # 9..10  : relative goal dx/dy, normalized to [-1, 1]
        # 11..14 : previous action one-hot
        # 15     : previous move succeeded (0/1)
        # 16     : previous action collided/blocked (0/1)
        # 17     : current tile visit count / max_steps
        #
        # 전체 지도는 제공하지 않는다.
    ...
    def _get_observation(self):
        ...
        # 현재 위치 기준 3x3. 장애물 또는 맵 바깥 = 1, 이동 가능 = 0.
        ...
        goal_relative = np.array(
            [
                (gx - ax) / float(self.grid_size - 1),
                (gy - ay) / float(self.grid_size - 1),
            ],
            dtype=np.float32,
        )

        previous_action = self._get_previous_action_one_hot()

        current_visits = np.array(
            [
                np.clip(
                    self.visit_count_map[ay, ax] / float(self.max_steps),
                    0.0,
                    1.0,
                )
            ],
        ...
        short_memory = np.array(
            [
                1.0 if self.previous_moved else 0.0,
                1.0 if self.previous_collision else 0.0,
            ],
            dtype=np.float32,
        )
    ...
    def step(
        ...
        self.previous_moved = bool(moved)
        self.previous_collision = bool(not moved)
...
if __name__ == "__main__":
    ...
    env = (
        PartialObstacleCompactMemoryEnv()
    )
